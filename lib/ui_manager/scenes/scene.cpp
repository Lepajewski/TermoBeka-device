...
std::unique_ptr<Scene> Scene::create_scene(SceneEnum target, std::shared_ptr<UISystemState> system_state)
{
    ...
    switch (target) {
        ...
        break;
        case SceneEnum::waiting_message: {
            result = std::make_unique<WaitingMessageScene>(system_state);
        }
        break;
        case SceneEnum::select_profile: {
            result = std::make_unique<SelectProfileScene>(system_state);
        }
        break;
        ...
        case SceneEnum::settings: {
            result = std::make_unique<SettingsScene>(system_state);
        }
        break;
        default:
        break;
