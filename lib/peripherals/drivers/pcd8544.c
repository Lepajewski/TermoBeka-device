...
static const char * const TAG = "PCD8544";
...
static esp_err_t pcd8544_init_control_pin() {
...
static esp_err_t pcd8544_init_spi() {
    esp_err_t err = ESP_OK;
...
esp_err_t pcd8544_init(pcd8544_config_t *pcd8544_config) {
    esp_err_t err = ESP_OK;
    config = *pcd8544_config;

    if ((err = pcd8544_init_control_pin()) != ESP_OK) {
        return err;
    }
    ...
    if ((err = pcd8544_init_spi()) != ESP_OK) {
        return err;
    }
    ...
    pcd8544_set_contrast(PCD8544_DEFAULT_CONTRAST);
...
static void pcd8544_check_queue_size() {
    if (trans_queue_size == PCD8544_TRANS_QUEUE_SIZE) {
        ...
        if (now - last_warned_at >= PCD8544_WARN_SUPPRESS_INTERVAL_MS) {
            TB_LOGW(TAG, "trans_queue_size reaches PCD8544_TRANS_QUEUE_SIZE(%d). Force syncing (may get some latency). Consider increasing PCD8544_TRANS_QUEUE_SIZE for less CPU usage", PCD8544_TRANS_QUEUE_SIZE);
            last_warned_at = now;
        }
        pcd8544_sync_and_gc();
    }
...
void pcd8544_set_contrast(uint8_t vop) {
    ...
    cmds[2] = 0b00010000 | PCD8544_DEFAULT_BIAS; // bias system
