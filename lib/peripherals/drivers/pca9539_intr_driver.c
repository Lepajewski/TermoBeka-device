...
static pca9539_pin_num pca9539_get_pin_from_port_change(pca9539_port_num port_num, uint8_t old_port_state, uint8_t new_port_state) {
    ...
    // printf("%s %d %" PRIu8 " %" PRIu8 "\n", __func__, (port_num << 3) | pos, old_port_state, new_port_state);
...
void PCA9539IntrTask(void *pvParameters) {
    ...
    uint8_t port_state_0 = 0;
    uint8_t port_state_1 = 0;
    ...
    pca9539_get_input_port_state(pca9539_cfg, PORT_0, &port_state_0);
    pca9539_get_input_port_state(pca9539_cfg, PORT_1, &port_state_1);

    while (1) {
        uint8_t new_port_state_0 = 0;
        uint8_t new_port_state_1 = 0;

        ulTaskNotifyTakeIndexed(0, pdTRUE, portMAX_DELAY);

        // it resets interrupt
        intr_evt.port_num = PORT_0;
        pca9539_get_input_port_state(pca9539_cfg, PORT_0, &new_port_state_0);
        intr_evt.pin_num = pca9539_get_pin_from_port_change(PORT_0, port_state_0, new_port_state_0);

        if (intr_evt.pin_num != PIN_READ_NEXT_PORT) {
            intr_evt.change_type = pca9539_get_pin_state_from_pin_change(intr_evt.pin_num, port_state_0, new_port_state_0);
            port_state_0 = new_port_state_0;
        } else {
            pca9539_get_input_port_state(pca9539_cfg, PORT_1, &new_port_state_1);
            intr_evt.port_num = PORT_1;
            intr_evt.pin_num = pca9539_get_pin_from_port_change(PORT_1, port_state_1, new_port_state_1);
            intr_evt.change_type = pca9539_get_pin_state_from_pin_change(intr_evt.pin_num, port_state_1, new_port_state_1);
            port_state_1 = new_port_state_1;
        }
